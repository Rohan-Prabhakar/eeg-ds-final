...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Project inputs
        query = self.query_proj(x).view(batch_size, channels, self.num_heads, self.head_dim)
        key = self.key_proj(x).view(batch_size, channels, self.num_heads, self.head_dim)
        value = self.value_proj(x).view(batch_size, channels, self.num_heads, self.head_dim)
        
        # Transpose for attention computation
        query = query.transpose(1, 2)  # [batch_size, num_heads, channels, head_dim]
        key = key.transpose(1, 2)  # [batch_size, num_heads, channels, head_dim]
        value = value.transpose(1, 2)  # [batch_size, num_heads, channels, head_dim]
        
        # Compute attention scores
        attention_scores = torch.matmul(query, key.transpose(-2, -1)) / self.scale.to(x.device)
        ...
        # Reshape and project
        context = context.transpose(1, 2).contiguous().view(batch_size, channels, features)
...
# DCRNN model with Multi-Head Attention
class DCRNN(nn.Module):
    def __init__(self, input_shape, num_classes=2, connectivity_type='correlation',
                 hidden_size=64, num_layers=2, threshold=0.5, bidirectional=True, 
                 dropout=0.5, num_attention_heads=8):
    ...
    def forward(self, x):
        ...
        for t in range(windows):
            ...
            attention_out = self.multi_head_attention(gcn_out)
            
            gcn_outputs.append(attention_out.unsqueeze(2))  # Add time dimension
    ...
    def extract_features(self, x):
        ...
        for t in range(windows):
            ...
            gcn_outputs.append(attention_out.unsqueeze(2))
